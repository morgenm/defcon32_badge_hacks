...
bool __attribute__((noinline)) sdCardInit(void)
{
	...
	if (mSD.flags.SD && sdPrvGetBits(respBuf, 16, 126, 2)) {	//v2 SD csd
		...
		if (!mSD.flags.SD && sdPrvGetBits(respBuf, 16, 126, 2) >= 2 && sdPrvGetBits(respBuf, 16, 122, 4) >= 4) {
			...
			if (VERBOSE)
				pr("MMC+ over 4GB not supported due to lack of testing\n");
	...
	if (VERBOSE)
		pr("read timeout: %u ticks, write %u + %u bytes\n", readToTicks, writeToTicks, toBytes);
	...
	if (sdPrvGetBits(respBuf, 16, 12, 1)) {
		...
		if (VERBOSE)
			pr("Card temporarily write protected\n");
		mSD.flags.RO = true;
	}
	...
	if (sdPrvGetBits(respBuf, 16, 13, 1)) {
		...
		if (VERBOSE)
			pr("Card permanently write protected\n");
		mSD.flags.RO = true;
	}
	
	if (VERBOSE)
		pr("CARD capacity: %u 512-byte blocks\n", mSD.numSec);
	...
	if (mSD.flags.SD) {		//some SD-specific things
		...
		if (!(respBuf[1] & 1)) {
			if (VERBOSE)
				pr("card claims to not support 1-bit mode...clearly SCR is wrong\n");
			return false;
		}
		...
		if ((hwFlags & SD_HW_FLAG_SUPPORT_4BIT) && (respBuf[1] & 4)) {
			...
			if (VERBOSE)
				pr("card support 4-wide bus. we'll do that\n");
		...
		if (0) {
			...
			if ((respBuf[16] & 0x0f) == 1) {
				...
				if (VERBOSE)
					pr("SD high speed mode supported\n sending cmd6 (to set HS mode)\n");
				
				if (!sdPrvCmd6(0x80000001, respBuf))
					return false;
				if ((respBuf[16] & 0x0f) == 1) {
					if (VERBOSE)
						pr("switch to high speed mode was a success\n");
					maxSpeed = 50000000;
				}
			}
			else {				//high speed mode not supported - falling back to 25.0MHz
				if (VERBOSE)
					pr("high speed mode not supported\n");
			}
		}
		else {				//high speed mode not supported - falling back to 12.5MHz
			if (VERBOSE)
				pr("SD v1.0: high speed mode not known yet\n");
			maxSpeed = 12500000;
		}
...
{
	return mSD.flags.inited ? mSD.numSec : 0;
}
