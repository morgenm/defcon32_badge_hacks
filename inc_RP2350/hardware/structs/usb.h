...
typedef struct {
    ...
    // 0x00800000 [23]    : ENDPOINT_ERROR (0): An endpoint has encounted an error
    ...
    // 0xf0000000 [31:28] : RETRY_COUNT_HI (0): Bits 9:6 of nak_retry count
    ...
    // 0x0000fc00 [15:10] : RETRY_COUNT_LO (0): Bits 5:0 of nak_retry_count
    ...
    // 0x80000000 [31]    : SWAP_DPDM (0): Swap the USB PHY DP and DM pins and all related controls and flip receive...
    // 0x00000010 [4]     : USBPHY_AS_GPIO (0): Use the usb DP and DM pins as GPIO pins instead of connecting them to the...
    ...
    // Used for debug only
    // 0x00000f00 [11:8]  : SPARE_FIX (0)
    // 0x00000080 [7]     : DEV_LS_WAKE_FIX (1): Device - exit suspend on any non-idle signalling, not qualified with a 1ms timer
    // 0x00000040 [6]     : DEV_RX_ERR_QUIESCE (1): Device - suppress repeated errors until the device FSM is next in the...
    // 0x00000020 [5]     : SIE_RX_CHATTER_SE0_FIX (1): RX - when recovering from line chatter or bitstuff errors, treat...
    // 0x00000010 [4]     : SIE_RX_BITSTUFF_FIX (1): RX - when a bitstuff error is signalled by rx_dasm, unconditionally...
    // 0x00000008 [3]     : DEV_BUFF_CONTROL_DOUBLE_READ_FIX (1): Device - the controller FSM performs two reads of the...
    // 0x00000004 [2]     : MULTI_HUB_FIX (0): Host - increase inter-packet and turnaround timeouts to accommodate...
    // 0x00000002 [1]     : LINESTATE_DELAY (0): Device/Host - add an extra 1-bit debounce of linestate sampling
    // 0x00000001 [0]     : RCV_DELAY (0): Device - register the received data to account for hub bit dribble before EOP
    ...
    // Device only
    // 0x001fffff [20:0]  : SOF_TIMESTAMP_RAW (0)
    io_ro_32 sof_timestamp_raw;

    _REG_(USB_SOF_TIMESTAMP_LAST_OFFSET) // USB_SOF_TIMESTAMP_LAST
    // Device only
    // 0x001fffff [20:0]  : SOF_TIMESTAMP_LAST (0)
    io_ro_32 sof_timestamp_last;

    _REG_(USB_SM_STATE_OFFSET) // USB_SM_STATE
    // 0x00000f00 [11:8]  : RX_DASM (0)
    // 0x000000e0 [7:5]   : BC_STATE (0)
    // 0x0000001f [4:0]   : STATE (0)
    io_ro_32 sm_state;

    _REG_(USB_EP_TX_ERROR_OFFSET) // USB_EP_TX_ERROR
    // TX error count for each endpoint
    // 0xc0000000 [31:30] : EP15 (0)
    // 0x30000000 [29:28] : EP14 (0)
    // 0x0c000000 [27:26] : EP13 (0)
    // 0x03000000 [25:24] : EP12 (0)
    // 0x00c00000 [23:22] : EP11 (0)
    // 0x00300000 [21:20] : EP10 (0)
    // 0x000c0000 [19:18] : EP9 (0)
    // 0x00030000 [17:16] : EP8 (0)
    // 0x0000c000 [15:14] : EP7 (0)
    // 0x00003000 [13:12] : EP6 (0)
    // 0x00000c00 [11:10] : EP5 (0)
    // 0x00000300 [9:8]   : EP4 (0)
    // 0x000000c0 [7:6]   : EP3 (0)
    // 0x00000030 [5:4]   : EP2 (0)
    // 0x0000000c [3:2]   : EP1 (0)
    // 0x00000003 [1:0]   : EP0 (0)
    ...
    // Watchdog that forces the device state machine to idle and raises an interrupt if the device stays in a state that...
    // 0x00100000 [20]    : FIRED (0)
    // 0x00080000 [19]    : RESET (0): Set to 1 to forcibly reset the device state machine on watchdog expiry
    // 0x00040000 [18]    : ENABLE (0)
    // 0x0003ffff [17:0]  : LIMIT (0)
