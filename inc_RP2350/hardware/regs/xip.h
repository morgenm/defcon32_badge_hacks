...
// =============================================================================
// Register    : XIP_CTRL
// Description : Cache control register. Read-only from a NonSecure context.
...
//               NonSecure software to break Secure flash reads if it has
...
//               and route all cached+NonSecure accesses to way 1 of the cache.
//
//               This partitions the cache into two half-sized direct-mapped
//               regions, such that NonSecure code can not observe cache line
...
// Description : When 0, NonSecure accesses to the cache maintenance address
//               window (addr[27] == 1, addr[26] == 0) will generate a bus
//               error. When 1, NonSecure accesses can perform cache maintenance
//               operations by writing to the cache maintenance address window.
//
//               Cache maintenance operations may be used to corrupt Secure data
//               by invalidating cache lines inappropriately, or map Secure
//               content into a NonSecure region by pinning cache lines.
...
//               tag memory before granting maintenance operations to NonSecure
//               code.
...
// Description : When 1, NonSecure accesses to the uncached, untranslated window
...
// Description : When 1, NonSecure accesses to the uncached window (addr[27:26]
...
// Description : When 1, enable the cache for NonSecure accesses. When enabled,
//               NonSecure XIP accesses to the cached (addr[26] == 0) window
