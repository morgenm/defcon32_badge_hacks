...
//               that is already in progress. If there is a chance that
//               memory-mapped transfers may be in progress, the busy flag
...
//               RX FIFO push will also contain 16 bits of data. The
...
// Description : Enforce a maximum assertion duration for chip select 0, in
//               units of 64 system clock cycles. If 0, the QMI is permitted to
//               keep the chip select asserted indefinitely when servicing
//               sequential memory accesses (see COOLDOWN).
//
...
// Description : After chip select 0 is deasserted, it remains deasserted for
//               half an SCK cycle (rounded up to an integer number of system
//               clock cycles), plus MIN_DESELECT additional system clock
//               cycles, before the QMI reasserts either chip select pin.
//
...
//               account for the round trip delay of the pads, and the
...
//               inserted after the M0_TIMING write to ensure the SCK divisor
...
// Description : Read command constants for memory address window 0.
//
...
// Description : Write command constants for memory address window 0.
//
...
// Description : Enforce a maximum assertion duration for chip select 1, in
//               units of 64 system clock cycles. If 0, the QMI is permitted to
//               keep the chip select asserted indefinitely when servicing
//               sequential memory accesses (see COOLDOWN).
//
...
// Description : After chip select 1 is deasserted, it remains deasserted for
//               half an SCK cycle (rounded up to an integer number of system
//               clock cycles), plus MIN_DESELECT additional system clock
//               cycles, before the QMI reasserts either chip select pin.
//
...
//               in the system clock frequency, a dummy access to either memory
//               window (and appropriate processor barriers/fences) must be
//               inserted after the M1_TIMING write to ensure the SCK divisor
//               change is in effect _before_ the system clock is changed.
...
// Description : Read command constants for memory address window 1.
//
...
// Description : Write command constants for memory address window 1.
//
